 ...
 *
 * WHAT THIS FINDS ON TRAITOR TRACE, and what it deliberately does not.
 *
 * It walks the WHOLE document, unlike `gate.ts`'s `auditControlBoundaries`,
 * which is scoped to `#app`. That is on purpose: the shared Crypto Lab top bar
 * is not this repo's to change — every lab in the fleet carries a
 * byte-identical copy of its markup and CSS — so its `.cl-btn` edge is measured
 * here, ratcheted in `nontext-baseline.ts`, and reported upward rather than
 * patched in one repo.
 *
 * `style.css` declares no `content` property at all, so the generated-content
 * half currently has nothing to find. It runs at every driven state regardless,
 * because the day someone replaces the user agent's `<details>` marker with a
 * `::before` glyph — the single most common way this fleet has grown an
 * unmeasurable non-colour cue — it will be measured on the day it lands.
 *
 * Three 1.4.11 cases on this page are out of reach of BOTH lists here and are
 * measured separately by `auditGraphics` in `gate.ts`: the subset-cover tree,
 * the probe strip and the collusion histogram. Their parts are shapes rather
 * than controls, so the `CONTROL` list below does not reach them, and two of the
 * three sit inside `aria-hidden` subtrees, where `contrast.ts` stops by design.
 * Yet which tree nodes are cover-set roots, which subscribers a probe gave duds,
 * and how often the tracer accuses an innocent subscriber ARE "parts of a
 * graphic required to understand the content" — they are the whole argument the
 * lab is making.
 */
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * Each gradient is *sampled* at the element's real location rather than
     * judged at its worst stop: linear by projecting onto the gradient line,
     * radial by distance from the centre over the farthest-corner radius. A
     * non-gradient layer (`url()`, `none`) is unmeasurable and paints nothing.
     *
     * This lab uses no gradient anywhere — every surface it paints is a flat
     * colour or a `color-mix()` — so this branch is dead code today. It is
     * shared verbatim with `contrast.ts` on purpose: the two oracles have to
     * agree about what a surface IS, and the one time they disagreed in this
     * fleet the result was a cyan disclosure triangle reported at 3.95:1 against
     * a page that is actually near-black.
     */
     ...
     *
     * A driven pass here walks four exhibits: a sixteen-cell decoder grid, a
     * sixteen-checkbox traitor picker, a probe transcript whose every row holds
     * sixteen one-character spans, and an SVG tree of 31 nodes. Without the
