 ...
 *
 *  - TEXT OVER A SURFACE AXE DECLINES TO RESOLVE. `.entry-bar` — the header
 *    entry a reader counts to watch the cover shrink — is
 *    `color-mix(in oklab, var(--accent) 18%, var(--bg-panel))`, and the hero
 *    aside is `color-mix(… 6%, transparent)`. axe files those under
 *    `incomplete`, which is where a violations-only gate never looks. Every
 *    verdict on this page also sits on a `.verdict-card`, which is `--bg-inset`
 *    inside a `--bg-panel` panel — 1.03:1 apart, so the ink is doing all the
 *    work and being sure of its ratio matters.
 ...
 *    at `.85`, the page's subtitle, measured for real rather than from its
 *    declared value.
 *
 ...
 * compute the ratio against the surface the text is genuinely sitting on rather
 * than against white.
 *
 ...
 *    boxes are `.tree-scroll` (`overflow-x: auto` around a `.tree-inner` with a
 *    declared `min-width: 640px`, so it overflows at every phone width and at
 *    some desktop ones), `.entry-bar` (`overflow: hidden` with a
 *    `text-overflow: ellipsis` over a hex wrap), and `.visually-hidden`, handled
 *    separately below by the `clip` guard. Content scrolled or ellipsed past
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND, and this
 *    lab is the case that needs it. `tree-view.ts` draws the subscriber tree as
 *    a real SVG: a `regionLayer` of tinted `<rect>` cover subsets FIRST, then
 *    the edges, then one `<g>` per node holding a `<circle>` and a `<text>`
 *    label. Those labels are painted over whichever region rect lies beneath
 *    them, so an ancestor-only walk would composite them onto the `<svg>`'s own
 *    transparent background instead and report a ratio nothing on screen has.
 *
 ...
 * rule. `style.css` declares no `content` property anywhere, so this page has no
 * generated content at all today; `nontext.ts` measures the class regardless, at
 * every driven state, so the first one added is measured on the day it lands —
 * including the day someone replaces `details.expert > summary`'s user-agent
 * marker with a `::before` glyph.
 *
 * SECOND, anything inside an `aria-hidden` subtree, which this walk skips for
 * the same reason axe does. On this page that boundary is load-bearing rather
 * than incidental, because this lab argues in pictures: `.probe-cells` (the
 * box's-eye view of one probe) and `.hist-bar` (how often the tracer accuses an
 * innocent subscriber) are both `aria-hidden` with prose beside them, and both
 * are graphics a sighted reader is asked to read. They are measured on purpose
 * by `auditGraphics` in `gate.ts`, at every driven state, precisely because this
 * walk and axe both stop at the attribute.
 */
 ...
 * uses. The one genuinely staged thing here is the probe transcript, which
 * `trace.ts` writes one `.probe-row` at a time with `dom.ts`'s `pause(420)`
 * between rows. Under the reduced-motion preference this gate asserts, `pause()`
 * reads `matchMedia` itself and resolves immediately, so the transcript appears
 * whole and there is no half-written state a scan could catch by accident.
 *
 * The parameter is kept because the narrowed form is what makes a short-lived
 * state measurable at all: a row that exists for 420ms is shorter than a full
 * axe pass, so a whole-page scan would catch it only sometimes, and a
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * gradient whose worst stop spans its element, and it is wrong for any
     * gradient taller than a screen. So each gradient is *sampled* at the text's
     * real location instead: linear by projecting onto the gradient line, radial
     * by distance from the centre over the farthest-corner radius. A
     * non-gradient layer (`url()`, `none`) is unmeasurable and paints nothing.
     *
     * THIS LAB HAS NO GRADIENT ANYWHERE. `html` and `body` both take a flat
     * `background-color: var(--bg)`, every panel surface is a flat
     * `--panel`/`--panel-2`, and the only non-flat fills are `color-mix()`
     * results, which `resolve()` handles as ordinary translucent colours. This
     * branch is therefore dead code TODAY, and is kept deliberately: a lab whose
     * backdrop is flat is exactly the one where a gradient added later would be
     * measured by a helper that had quietly stopped modelling them.
     */
     ...
     *
     * A driven pass here walks the whole lab, and the expensive part is that every
     * revocation rebuilds a sixteen-cell decoder grid, a header list, a
     * three-row comparison table and a full verdict card, beside a
     * sixteen-checkbox traitor picker, a probe transcript whose every row holds
     * SIXTEEN one-character `.probe-cell` spans, and an SVG tree of 31 nodes —
     * so a single state is hundreds of sibling elements all re-walking the same
     ...
     * that is `.tree-scroll` (`overflow-x: auto` around a `min-width: 640px`
     * inner box), `.entry-bar` (`overflow: hidden` with an ellipsis over a hex
     * wrap), and `.visually-hidden`, which is `overflow: hidden` at 1x1 px. The
     * last is caught by the zero-area `clip` guard below as well, and
     * belt-and-braces is right there because it carries the caption of the
     * header-size table.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. Here that is all six
      // `details.expert` disclosures, every one of which ships shut and holds
      // the construction notes for its exhibit. The gate opens each by clicking
      // its <summary>, which is the route a reader has, rather than setting
      // `.open` from script — which is exactly what the spec this replaces did
      // to all six at once, before its single scan.
      ...
      //
      // THIS LAB USES THE CLIP IDIOM FOR REAL, which is why the guard is
      // load-bearing here rather than inherited. `style.css` defines
      // `.visually-hidden` as `position:absolute; width:1px; height:1px;
      // overflow:hidden; clip:rect(0 0 0 0)`, and it carries the `<caption>` of
      // the header-size comparison table — the table that is the whole numeric
      // argument of Exhibit 2. It has a rect and passes `checkVisibility()`
      // while the compositor draws exactly zero pixels of it, so a walk without
      // this guard reports a ratio for ink that was never laid down. It is
     ...
     *
     * What this lab hides is a closed set with four members. Two are plainly
     * decorative: the ✓/🔒 glyph in each `.sub-cell` and the ·/🔑/🚫/✕ glyph on
     * each tree leaf button — each sits beside its own words AND is repeated in
     * that button's `aria-label`, so nothing carrying meaning is lost.
     *
     * THE OTHER TWO ARE NOT DECORATIVE, AND ARE STATED PLAINLY: `.probe-cells`,
     * the box's-eye view of which subscribers a probe gave real keys and which
     * it gave duds; and `.hist-bar`, the length of every bar in the collusion
     * histogram. Both are `aria-hidden` for a good reason — the same facts are
     * printed as prose immediately beside them, which is a better alternative
     * than a screen reader crawling sixteen one-character spans — but the
     * consequence is that two of this lab's four pictures sit in the one place
     * BOTH oracles stop looking. They are therefore measured on purpose, by
     * `auditGraphics` in `gate.ts`, at every driven state.
     *
     * Nothing here hides a VALUE. No verdict, no key fingerprint, no header
     * count and no accusation is inside an `aria-hidden` subtree — which was
     * checked rather than assumed, because that is the shared blind spot where
     * BOTH oracles stop, and it is the one place a live readout can hide from a
     ...
     *
     * This lab builds a real SVG figure — `tree-view.ts` appends `<line>`,
     * `<rect>`, `<circle>`, `<g>` and `<text>` through `createElementNS` — so
     * the guard is live rather than hypothetical. The mistake it catches is one
     * line away here: `svgEl()` takes only attributes, and a caller who appends
     * a label to a `<g>` instead of to a `<text>` gets a node that is in the
     * DOM, has a rect, and paints nothing, which a walk without this guard would
     * report as a 1:1 failure for ink never laid down.
     */
     ...
     *
     * It is a no-op on this page TODAY, and that was measured rather than
     * assumed: `html` and `body` both set `background-color: var(--bg)` and
     * neither sets a height, so `<html>`'s box already spans the document and
     * the geometry-aware walk would have found it anyway. It is kept because
     * this page runs eight sections deep and the day a `height: 100%` appears on
     * either box — or the day `html`'s background is dropped as redundant with
     * `body`'s — everything below the first screen would report against a white
     * page that does not exist, in a lab whose dark palette is `#0b1118`.
     */
